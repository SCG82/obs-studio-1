...
bundle_dylibs() {
    ...
    if [ "${MACOS_CEF_BUILD_VERSION:-${CI_MACOS_CEF_VERSION}}" -le 3770 ]; then
        BUNDLE_PLUGINS+=( ./OBS.app/Contents/PlugIns/obs-browser-page )
    fi
...
prepare_macos_bundle() {
    ...
    if [ -d ./OBS.app/Contents/Resources/data/obs-scripting ]; then
        for f in ./OBS.app/Contents/Resources/data/obs-scripting/*.*; do
            /bin/mv "$f" ./OBS.app/Contents/MacOS/
        done
...
codesign_bundle() {
    ...
    if [ -d "./OBS.app/Contents/Frameworks/Chromium Embedded Framework.framework" ]; then
        local cef_path="./OBS.app/Contents/Frameworks/Chromium Embedded Framework.framework"
        for f in "./OBS.app/Contents/Frameworks/Chromium Embedded Framework.framework/Libraries/"*.dylib; do
            echo "* code-signing $f"
